...
pip3 install --quiet requests Pillow 2>/dev/null || pip3 install --break-system-packages --quiet requests Pillow 2>/dev/null || {
    ...
    pip3 install --user --quiet requests Pillow
}
...
echo "To test full sync (creates draft):"
echo "  cd $SITE_DIR && python3 wechat_sync.py --draft-only"
echo ""
echo "To test auto-publish:"
echo "  cd $SITE_DIR && python3 wechat_sync.py"
echo ""
echo "Cron runs daily at 22:00 server time."
